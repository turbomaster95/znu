...
SRC=$(realpath "$SRCTREE/lib/fatfs/source" 2>/dev/null)
INC="-I$SRCTREE/lib/uacpi/include -I$SRCTREE/lib/libc/include -I$SRCTREE/include -I$SRCTREE/lib/fatfs/source"
INTERNAL_INC=$($CC -print-file-name=include)
OUT="lib/fatfs/.ffs_out"
FINAL_OBJ="$OBJTREE/lib/fatfs/ffs_lib.o"

mkdir -p "$OUT"

# 4. Find core files in the Source Tree
if [ -z "$SRC" ] || [ ! -d "$SRC" ]; then
    echo "  ERROR: Source directory ($SRCTREE/lib/fatfs/src) not found."
