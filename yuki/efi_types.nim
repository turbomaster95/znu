...
type
  ...
  EfiBootServices* = object
    ...
    stall*: proc (this: ptr EfiBootServices, microseconds: uint): EfiStatus {.cdecl.}  # Stall
...
type
  Smbios3EntryPoint* {.packed.} = object
    anchor*: array[5, char]      # _SM3_
    checksum*: uint8
    length*: uint8
    majorVersion*: uint8
    minorVersion*: uint8
  ...
  SmbiosHeader* {.packed.} = object
    `type`*: uint8
    length*: uint8
    handle*: uint16

  # Type 11: OEM Strings
  SmbiosType11* {.packed.} = object
    header*: SmbiosHeader
    count*: uint8 # Number of strings
